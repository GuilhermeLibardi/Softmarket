...
public class EditarReceitaController implements Initializable {
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ingredientesEstoque = e.getEstoqueI();
        listaIngredientes.setCellFactory(param -> new ListCell<Ingredientes>() {
            @Override
        ...
        txtPdc.setDisable(true);
        txtPdv.setDisable(true);
        estado = 0;
    }
    ...
    void submit() {
        switch (estado) {
            case 0:
                //Pega o cod de barras
                codBarras = txtCodBarras.getText();
                try {
                    Receitas r = e.pesquisarReceita(codBarras);
                    txtNome.setText(r.getNome());
                    txtPdc.setText(String.valueOf(r.getValorCusto()));
                    txtPdv.setText(String.valueOf(r.getValorVenda()));
                    txtNome.setDisable(false);
                    txtPdc.setDisable(false);
                    txtPdv.setDisable(false);
                    estado++;
                } catch (ProdutoNaoEncontradoException e1) {
                    Alert alerta = new Alert(Alert.AlertType.ERROR);
                    alerta.setTitle("Receita não encontrada");
                    alerta.setHeaderText("Erro ao pesquisar esta receita no banco de dados");
            ...
            case 1:
                ...
                //Popula a lista da outra tela

                ReceitaContemIngredientes rci1 = new ReceitaContemIngredientes();
                rci1.setCodReceita(txtCodBarras.getText());
                for (int i = 0; i < ingredientesEstoque.size(); i++) {
                    for (Ingredientes ing : rci1.IngredientesEscolhidosFromReceitaCod()) {
                        if (ing.getCodigo().equals(ingredientesEstoque.get(i).getCodigo())) {
                            listaIngredientes.getSelectionModel().select(i);
                        }
                    }
                }
            ...
            case 2:
                ...
                ingredientesEscolhidos.forEach(ingredientes -> ingredientes.setPeso(0));
